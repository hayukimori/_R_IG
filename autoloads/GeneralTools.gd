...
func request_server_code(email: String) -> Dictionary:
	var url: String = Routes.get_route(Routes.ENDPOINT_FORGOT_PASSWD)
...
func request_verify_code(email: String, code: String) -> Dictionary:
	var url := Routes.get_route(Routes.ENDPOINT_VERIFY_CODE)
	var generic_error := {"error": "Verify email or code and try again."}

	# Validates email and six-digit code.
	var email_validation := GlobalValidations.validate_email(email)
	var code_validation := GlobalValidations.validate_code(code)
...
func request_password_reset(token: String, password: String, confirm_password: String) -> Dictionary:
	var url := Routes.get_route(Routes.ENDPOINT_RESET_PASSWD)
	var error_ret := {"error": "Could not complete request"}

	# Validates password and confirm_password
	var password_validation := GlobalValidations.validate_password(password)
	var c_password_validation := GlobalValidations.validate_password(confirm_password)
...
func simple_request(
	...
	var validation = GlobalValidations.validate_url(url)
	if !validation:
